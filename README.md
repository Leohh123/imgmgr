...
规则导出文件为 JSON 格式，包含规则 ID、父规则 ID、规则名称、规则类型、规则内容、匹配目标、启用状态、优先级、允许冲突、大小写设置、全字匹配设置和备注。

导入 JSON 时会覆盖当前所有规则，并清空已有规则命中记录。导入前程序会校验规则字段、父规则引用和规则树循环；导入成功后会刷新规则树并重新计算规则命中。
