...
- 支持按当前规则筛选，并可切换为“仅当前规则”以排除任意子规则命中的图片。
- 支持命中解释页，显示规则路径和冲突原因。
...
9. 在规则树中选中规则后，左侧筛选区会按该规则筛选图片；勾选“仅当前规则”时，只显示当前规则独占的图片。
10. 通过“项目 -> 导出规则为 JSON”备份当前规则。
11. 通过“项目 -> 从 JSON 导入规则并覆盖”还原规则。
12. 通过“项目 -> 重算规则命中”刷新命中状态和冲突检测。
13. 在右侧图片预览页查看完整图片和通道效果。

## 筛选说明

左侧筛选区支持通配符和正则表达式两种规则类型，可选择匹配目标、是否区分大小写、是否全字匹配，并可通过“已分类”“未分类”“冲突”“多重命中”控制状态过滤。

在规则树中选中某条规则后，“仅当前规则”复选框会启用：

- 未勾选：显示命中当前规则的所有图片，包括同时命中其子规则的图片。
- 已勾选：只显示命中当前规则、且不命中任意子规则的图片。
